...
async function seedAdmin() {
  ...
  if (env === 'production' && process.env.ALLOW_SEED_IN_PRODUCTION !== 'true') {
    console.error('❌ Refusing to seed in production. Set ALLOW_SEED_IN_PRODUCTION=true to override.');
    process.exit(1);
  }
  ...
  if (!adminEmail || !adminPass) {
    console.error('❌ SEED_ADMIN_EMAIL and SEED_ADMIN_PASSWORD must be set. Refusing to use defaults.');
    process.exit(1);
  }

  // ✅ Log target environment so operator can confirm before proceeding:
  console.log(`⚠️  Seeding admin in environment: ${env}`);
  console.log(`⚠️  Target host: ${process.env.DATABASE_URL?.split('@')[1] || 'unknown'}`);
