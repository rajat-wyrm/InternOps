...
function toCsv(rows) {
  const cols = [
    'full_name', 'email', 'role', 'department_name', 'phone', 
    'location', 'college', 'course', 'position', 'joining_date', 'internship_status',
  ];
...
async function routes(fastify) {
  fastify.get('/members', { preHandler: [auth, rbac(...MANAGER_ROLES)] }, async (req) => repo.getTeamMembers(req.user.id));

  fastify.get('/members/export', { preHandler: [auth, rbac(...MANAGER_ROLES)] }, async (req, reply) => {
    const members = await repo.getTeamMembers(req.user.id);
    reply.header('Content-Type', 'text/csv');
    reply.header('Content-Disposition', 'attachment; filename="team-members.csv"');
    return toCsv(members);
  });

  fastify.get('/pending-proofs', { preHandler: [auth, rbac(...MANAGER_ROLES)] }, async (req) => repo.getPendingProofs(req.user.id));

  fastify.post('/members', { preHandler: [auth, rbac(...MANAGER_ROLES)] }, async (req, reply) => {
    ...
    const result = await withHierarchyTx([managerId], async (client) => {
      ...
      const managerRole = managerId === req.user.id ? req.user.role : await repo.getUserRole(managerId, client);
      if (!managerRole) return { errStatus: 400, errMessage: 'Manager not found' };

      if (ROLE_RANK[data.role] === undefined || ROLE_RANK[data.role] >= ROLE_RANK[managerRole]) {
        return { errStatus: 400, errMessage: `You can only add members below your own role (${managerRole})` };
      }
      if (await repo.emailExists(data.email, client)) return { errStatus: 409, errMessage: 'Email already exists' };

      const member = await repo.createMember({ ...data, manager_id: managerId }, client);
  ...
  fastify.get('/members/:id', { preHandler: [auth, rbac(...MANAGER_ROLES), ownership('id')] }, async (req, reply) => {
    const member = await repo.getMemberById(req.params.id);
    return member || reply.status(404).send({ error: 'Member not found' });
  });

  fastify.get('/members/:id/history', { preHandler: [auth, rbac(...MANAGER_ROLES), ownership('id')] }, async (req) => repo.getMemberHistory(req.params.id));

  fastify.patch('/members/:id', { preHandler: [auth, rbac(...MANAGER_ROLES), ownership('id')] }, async (req, reply) => {
  ...
  fastify.patch('/members/:id/status', { preHandler: [auth, requireFreshRole, rbac(...MANAGER_ROLES), ownership('id')] }, async (req, reply) => {
  ...
  fastify.patch('/members/:id/role', { preHandler: [auth, requireFreshRole, rbac(...MANAGER_ROLES), ownership('id')] }, async (req, reply) => {
    const { role } = z.object({ role: z.enum(ASSIGNABLE_ROLES) }).parse(req.body);
    if (req.params.id === req.user.id) return reply.status(403).send({ error: 'Cannot change own role' });
    if (req.user.role !== 'ADMIN' && ROLE_RANK[role] >= ROLE_RANK[req.user.role]) {
      return reply.status(403).send({ error: `Only assign roles below your own (${req.user.role})` });
    }
    ...
    const result = await withHierarchyTx([req.params.id], async (client) => {
      ...
      if (reportRoles.reduce((max, r) => Math.max(max, ROLE_RANK[r] ?? 0), -1) >= ROLE_RANK[role]) {
        return { errStatus: 400, errMessage: 'New role must outrank existing reports' };
      }
  ...
  fastify.patch('/members/:id/manager', { preHandler: [auth, rbac(...MANAGER_ROLES), ownership('id')] }, async (req, reply) => {
    const { manager_id } = z.object({ manager_id: z.string().uuid() }).parse(req.body);
    if (manager_id === req.params.id) return reply.status(400).send({ error: 'Cannot be own manager' });
    ...
    const result = await withHierarchyTx([req.params.id, manager_id], async (client) => {
      ...
      if (manager_id !== req.user.id && req.user.role !== 'ADMIN' && !(await checkHierarchyAccess(req.user.id, manager_id, client))) {
        return { errStatus: 403, errMessage: 'Chosen manager not in team' };
      }
      const managerRole = manager_id === req.user.id ? req.user.role : await repo.getUserRole(manager_id, client);
      if (!managerRole || ROLE_RANK[member.role] >= ROLE_RANK[managerRole]) return { errStatus: 400, errMessage: 'Manager must outrank member' };
      if (await checkHierarchyAccess(req.params.id, manager_id, client)) return { errStatus: 400, errMessage: 'Cycle detected' };
