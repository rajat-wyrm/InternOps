...
app.register(async function sanitize(instance) {
...
app.register(require("@fastify/static"), {
  root: require("path").join(__dirname, "..", config.uploadDir),
  prefix: "/uploads/",
});
...
app.get("/fallback", async (req, reply) =>
  reply
    .type("text/html")
    .send(
      "<html><body style=font-family:sans-serif;padding:2em><h1>InternOps API</h1><a href=/docs>Swagger</a></body></html>",
    ),
);
...
app.addHook("onRequest", async (req) =>
  req.log.info({ reqId: req.id, method: req.method, url: req.url }, "incoming"),
);
app.setErrorHandler((err, req, reply) => {
  if (err.name === "ZodError" || err.issues) {
    ...
    return reply
      .status(400)
      .send({ error: "Validation failed", details: err.errors || err.issues });
  }
  if (err.statusCode) {
    req.log.warn(err);
