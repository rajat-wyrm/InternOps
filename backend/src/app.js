...
app.register(
  require('./modules/github-sync/routes'),
  {
...
// ---------------------------------------------------------------------------
// Response logging / deferred audit
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    writeStartupSummary({
      ...
      degradedFeatures:
        getRedisDegradedFeatures(),
      queue:
        bulkJobQueue.getStatus(),
      integrations:
        integrationStatus(config),
      port: config.port,
    });
  } catch (err) {
    app.log.error(err);
