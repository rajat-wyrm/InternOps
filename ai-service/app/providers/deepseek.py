...
class DeepSeekProvider(BaseAIProvider):
    ...
    async def generate_text(self, prompt: str, temperature: float = 0.7, **kwargs) -> str:
        ...
        try:
            ...
            raise ProviderAPIError(
                f"Unexpected response payload from DeepSeek: {e}", self.provider_name
            )
    ...
    async def generate_json(
        ...
        try:
            ...
            raise ProviderAPIError(
                f"Failed to parse structured JSON from DeepSeek: {e}", self.provider_name
            )
            ...
            try:
                async with client.stream(
                    ...
                    if response.is_error:
                        ...
                        raise ProviderAPIError(
                            f"DeepSeek API error: {body.decode(errors='replace')}",
                            self.provider_name,
                ...
                raise ProviderTimeoutError(
                    "DeepSeek API request timed out", self.provider_name
                )
            except httpx.RequestError as e:
                raise ProviderAPIError(
                    f"Network error connecting to DeepSeek API: {e}", self.provider_name
                )
    ...
    async def _read_json_with_limit(self, response: httpx.Response) -> Dict[str, Any]:
        ...
        async for chunk in response.aiter_bytes():
            ...
            if received > MAX_RESPONSE_BYTES:
                raise ProviderAPIError(
                    f"DeepSeek response exceeded {MAX_RESPONSE_BYTES} bytes",
                    self.provider_name,
                )
            chunks.append(chunk)
        try:
            ...
            raise ProviderAPIError(
                f"DeepSeek returned invalid JSON: {e}", self.provider_name
            )
