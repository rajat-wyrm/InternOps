...
class NvidiaProvider(BaseAIProvider):
    ...
    async def generate_chat(self, messages: list[dict], temperature: float = 0.7, **kwargs) -> str:
        ...
        try:
            ...
            raise ProviderAPIError(
                f"Unexpected response payload from Nvidia: {e}", self.provider_name
            )
    ...
    async def generate_json(
        ...
        try:
            ...
            raise ProviderAPIError(
                f"Failed to parse structured JSON from Nvidia: {e}", self.provider_name
            )
            ...
            try:
                async with client.stream(
                    ...
                    if response.is_error:
                        ...
                        raise ProviderAPIError(
                            f"Nvidia API error: {body.decode(errors='replace')}",
                            self.provider_name,
                            status_code=response.status_code,
                        )
                    return await self._read_json_with_limit(response)
            except httpx.TimeoutException:
                raise ProviderTimeoutError("Nvidia API request timed out", self.provider_name)
            except httpx.RequestError as e:
                raise ProviderAPIError(
                    f"Network error connecting to Nvidia API: {e}", self.provider_name
                )
    ...
    async def _read_json_with_limit(self, response: httpx.Response) -> Dict[str, Any]:
        ...
        async for chunk in response.aiter_bytes():
            ...
            if received > MAX_RESPONSE_BYTES:
                raise ProviderAPIError(
                    f"Nvidia response exceeded {MAX_RESPONSE_BYTES} bytes",
                    self.provider_name,
                )
            chunks.append(chunk)
        try:
