...
export default function Attendance() {
  ...
  const selectedName = viewUserId === user?.id
    ? 'Me'
    : team.find(m => m.id === viewUserId)?.full_name || team.find(m => m.id === viewUserId)?.email || ''
  ...
  return (
    <div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm mb-4">
        <label className="block text-xs text-gray-500 mb-1">View attendance of</label>
        {isManager ? (
      ...
      {error && <p className="text-red-500">{error.response?.data?.error || 'Failed to load attendance'}</p>}
      {records && (
        records.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-8 text-center text-gray-500">No attendance records for {selectedName || 'this user'}.</div>
        ) : (
