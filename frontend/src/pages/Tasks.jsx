...
import {
  Target, Briefcase, Camera, MessageCircle, ThumbsUp, PlaySquare, Upload, CheckCircle, Link as LinkIcon, Clock, Plus, X, Trash2,
} from 'lucide-react';
...
export default function Tasks() {
  ...
  const submitMutation = useMutation({
    mutationFn: ({ taskId, file, actions }) => {
      const form = new FormData();
      form.append('task_id', taskId);
      form.append('image', file);
      form.append('did_comment', actions.didComment ? 'true' : 'false');
      form.append('did_repost', actions.didRepost ? 'true' : 'false');
      form.append('did_share', actions.didShare ? 'true' : 'false');
  ...
  const handleUpload = (e, taskId) => {
    ...
    submitMutation.mutate({ 
      taskId, 
      file, 
      actions: { didComment, didRepost, didShare } 
    });
  };

  const overdue = (d) => new Date(d) < new Date();
  const atLeastOneChecked = didComment || didRepost || didShare;
    ...
    <div className="animate-fade-in-up">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title="No tasks yet"
          text={canCreateTask ? 'Create a campaign to get started.' : 'New tasks will appear here.'}
        />
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {t.target_platform && <Badge color="purple">{t.target_platform}</Badge>}
                      {t.deadline && (
                ...
                <div className="flex flex-col gap-4 mt-5 pt-4 border-t border-slate-200 dark:border-slate-700">
                  {/* Engagement Action Configuration Selection for Interns */}
                  {user?.role === 'INTERN' && (
                    <div className="flex flex-wrap gap-4 items-center bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                      ...
                      <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-300 cursor-pointer">
                        <input 
Use code with caution.type="checkbox"checked={didRepost}onChange={(e) => setDidRepost(e.target.checked)}className="rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 dark:border-slate-600 dark:bg-slate-700 w-4 h-4"/>Repost)}{canVerify && (<Btnvariant="outline"className="rounded-2xl"onClick={() => setSelectedTask(selectedTask === t.id ? null : t.id)}>{selectedTask === t.id ? 'Hide proofs' : 'View proofs'})}{user?.role === 'INTERN' && (Submit Proof)}{selectedTask === t.id && (Proof submissions{proofs?.length || 0} submission{proofs?.length === 1 ? '' : 's'}{!proofs?.length ? (No submissions yet.) : (proofs.map((p) => ({p.image_path && (() => {const normalized = p.image_path.replace(/\/g, '/').replace(/^/+/, '');const base = (import.meta.env.VITE_API_BASE_URL || '').replace(//+$/, '');const src = base ? ${base}/${normalized} : /${normalized};return (<imgsrc={src}alt="proof"className="w-14 h-14 rounded-2xl object-cover border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900"onError={(e) => { e.currentTarget.style.visibility = 'hidden'; }}/>);})()}<Badge color={p.status === 'VERIFIED' ? 'green' : 'yellow'}>{p.status}{p.did_comment && Comment}{p.did_repost && Repost}{p.did_share && Share}Intern: {p.intern_name || p.intern_email || ${p.intern_id.slice(0, 8)}…}{canVerify && p.status === 'PENDING' && (<Btnvariant="success"className="rounded-2xl"onClick={() => verifyMutation.mutate(p.id)}> Verify)}{user?.role === 'ADMIN' && (<Btnvariant="outline"className="rounded-2xl text-red-500 border-red-200 hover:bg-red-50 dark:hover:bg-red-950/30"onClick={() => {if (confirm('Delete this proof? This cannot be undone.')) {deleteMutation.mutate(p.id);}}}> Delete)})))})});})})});}
