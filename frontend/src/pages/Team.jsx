...
// Which role a manager may directly create (mirrors backend roleStepMap).
const NEXT_ROLE = { ADMIN: ['SENIOR_TL'], SENIOR_TL: ['TL'], TL: ['CAPTAIN'], CAPTAIN: ['INTERN'] }
...
function AddMemberModal({ onClose }) {
  ...
  const allowedRoles = NEXT_ROLE[user?.role] || []
...
export default function Team() {
  ...
  const { user } = useAuthStore()
  const canAdd = (NEXT_ROLE[user?.role] || []).length > 0
