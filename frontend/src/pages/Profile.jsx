...
export default function Profile() {
  ...
  function handleAvatarChange(e) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      setAvatarError('Invalid file type. Only JPEG, PNG, WebP, and GIF are allowed.')
      e.target.value = ''
      return
    }
    if (file.size > MAX_SIZE) {
      setAvatarError('File is too large. Maximum allowed size is 5MB.')
      e.target.value = ''
      return
    }
    setAvatarError('')
  ...
  return (
    <div className="max-w-3xl">
      ...
      {avatarError && <div className="bg-red-50 text-red-700 px-4 py-2.5 rounded-xl mb-4 animate-fade-in">{avatarError}</div>}
        ...
        <div className="px-6 pb-6 -mt-10 flex items-end gap-4 flex-wrap">
          <div className="relative">
            ...
            <label className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-white shadow flex items-center justify-center cursor-pointer hover:scale-110 transition text-sm" title="Change avatar">
              📷
              <input type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" onChange={handleAvatarChange} />
            </label>
          </div>
          <div className="pb-1">
