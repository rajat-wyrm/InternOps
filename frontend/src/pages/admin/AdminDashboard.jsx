...
export default function AdminDashboard() {
  ...
  return (
    <div>
      <PageHeader title="User Management" icon="🛡️" subtitle="Manage all platform accounts" />

      {isLoading ? <Spinner /> : (
        <>
          <Table head={['User', 'Role', 'Status', 'Actions']}>
            {data?.map(u => (
              <tr key={u.id} className="border-t hover:bg-indigo-50/40 transition">
                <td className="p-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center text-xs font-bold">{initials(u)}</div>
                    <div>
                ...
                <td className="p-3"><Badge color={u.suspended ? 'red' : 'green'}>{u.suspended ? 'Suspended' : 'Active'}</Badge></td>
                <td className="p-3">
                  <div className="flex gap-2">
                    {u.suspended
                      ? <Btn variant="success" onClick={() => activateMut.mutate(u.id)}>Activate</Btn>
                      : <Btn variant="warning" onClick={() => suspendMut.mutate(u.id)}>Suspend</Btn>}
                    <Btn variant="danger" onClick={() => { if (confirm('Delete this user?')) deleteMut.mutate(u.id) }}>Delete</Btn>
                  </div>
                </td>
              </tr>
            ))}
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <Btn variant="outline" onClick={() => setPage(p => p + 1)} disabled={!data || data.length < 10}>Next →</Btn>
          </div>
        </>
      )}
    </div>
