    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-animated-gradient bg-gradient-to-br from-indigo-600 via-purple-600 to-fuchsia-600 animate-gradient-shift p-4">
      ...
      <div className="relative w-full max-w-md animate-pop-in">
        ...
        <div className="glass rounded-3xl border border-white/20 shadow-2xl p-8">
          {message && <div className="bg-green-500/20 border border-green-300/40 text-green-50 text-sm rounded-xl px-4 py-2.5 mb-4">{message}</div>}
          {error && <div className="bg-red-500/20 border border-red-300/40 text-red-50 text-sm rounded-xl px-4 py-2.5 mb-4">{error}</div>}
          <form onSubmit={(e) => { e.preventDefault(); resetMut.mutate({ token, newPassword }) }} className="space-y-4">
            <input type="text" placeholder="Reset token" value={token} onChange={e => setToken(e.target.value)} required
              className="w-full px-4 py-3 rounded-xl bg-white/15 border border-white/25 text-white placeholder-white/50 focus:bg-white/25 focus:ring-2 focus:ring-white/60 outline-none transition" />
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/60">🔒</span>
              <input type="password" placeholder="New password (min 8)" value={newPassword} onChange={e => setNewPassword(e.target.value)} required
          ...
          <div className="mt-5 text-center">
            <Link to="/login" className="text-white/80 hover:text-white text-sm hover:underline">← Back to login</Link>
          </div>
        </div>
      </div>
    </div>
