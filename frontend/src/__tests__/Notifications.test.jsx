...
describe('Notifications Page Optimistic UI Updates', () => {
  ...
  it('updates the unread count cache immediately when a notification is marked as read', async () => {
    const mockNotifications = [
      {
        id: 101,
        message: 'First unread alert',
    ...
    const queryClient = createTestQueryClient();
    queryClient.setQueryData(['notifications', 'unread-count'], { unread: 2 });
    ...
    await waitFor(() => {
      expect(
        queryClient.getQueryData(['notifications', 'unread-count'])
      ).toEqual({
        unread: 1,
      });
    });
  ...
  it('updates the unread count cache to zero when all notifications are marked as read', async () => {
    const mockNotifications = [
      {
        id: 101,
        message: 'First unread alert',
